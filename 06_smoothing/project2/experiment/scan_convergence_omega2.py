...
固定設定:
  参照画像 : ref_w1_15_w2_15.jpg
  初期ω₁  : 15°（真値固定）
  初期ω₃  : 0°（固定）
  真値     : ω₁=15°, ω₂=15°, ω₃=0°
  使用バイナリ: ./build/lm_scan_omega2
...
BINARY     = "./build/lm_scan_omega2"
REF_PATH   = "images/reference/ref_w1_15_w2_15.jpg"
RESULT_CSV = "results/scan_convergence_omega2.csv"
RESULT_PNG = "results/scan_convergence_omega2.png"


# ── バイナリのビルド確認 ──────────────────────────────────────────────────────
def build_binary():
    print("lm_scan_omega2 をビルド中 (make lm_scan_omega2)...")
    ret = subprocess.run(["make", "lm_scan_omega2"],
...
def run_lm_single(init_deg):
    """
    lm_scan_omega2 を init_deg (ω₂初期値) で実行し結果を返す。
    ...
    try:
        proc = subprocess.run(
            [BINARY, f"{init_deg:.1f}", conv_path, sum_path, REF_PATH],
        ...
        # サマリCSV を解析
        # 形式: init_deg,iter,final_E,frob_err,omega2_est_deg,R00..R22
                ...
                for line in f:
                    ...
                    if len(parts) >= 5:
                        try:
                            ...
                            omega2_est         = float(parts[4])
                            result['frob_err']         = frob_err
                            result['final_omega2_deg'] = omega2_est
                            result['error_deg']        = omega2_est - TRUE_DEG
    ...
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(future_to_deg):
            ...
            if res['converged_true']:
                status = (f"○収束(真)  推定ω₂={res['final_omega2_deg']:+7.3f}°"
                          f"  誤差={res['error_deg']:+7.3f}°  Frob={res['frob_err']:.5f}")
            elif res['converged']:
                status = (f"△収束(局)  推定ω₂={res['final_omega2_deg']:+7.3f}°"
...
def save_plot(results):
    ...
    for i in range(len(results)):
        if conv_true[i]:
            bar_vals.append(1); bar_cols.append('#2ecc71')
        elif conv_loc[i]:
            bar_vals.append(1); bar_cols.append('#f39c12')
        else:
            bar_vals.append(1); bar_cols.append('#e74c3c')
    ...
    ax1.set_yticks([])
    ax1.set_title(
        f'LM法 収束域スキャン（w1=15固定, w3=0固定, 真値 w2={TRUE_DEG:.0f}deg,'
    ...
    ax2.set_ylabel('最終誤差 (推定 w2 - 真値 w2) [deg]', fontsize=10)
...
def main():
    ...
    if args.single is not None:
        ...
        if res['converged_true']:
            status = (f"○真値収束  推定ω₂={res['final_omega2_deg']:+.3f}°"
                      f"  誤差={res['error_deg']:+.3f}°  Frob={res['frob_err']:.5f}")
        elif res['converged']:
            status = f"△局所収束  推定ω₂={res['final_omega2_deg']:+.3f}°  Frob={res['frob_err']:.5f}"
        else:
    ...
    print(f"  真値        : ω₂ = {TRUE_DEG:.1f}°  (ω₁=15°固定, ω₃=0°固定)")
