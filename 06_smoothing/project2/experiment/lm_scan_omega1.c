 ...
 *
 * 使い方:
 *   ./build/lm_scan_omega1 <init_omega1_deg> <収束CSV> <サマリCSV>
 *
 * 固定設定:
 ...
 *   真値     : (ω₁, ω₂, ω₃) = (15°, 30°, 0°)
 *
 * 符号規約（scan_3param.c / lm_estimate.c と同じ）:
...
static const double TRUE_W2_DEG = 30.0;
 ...
 *   omega_internal = θ/(2 sinθ) * [R21-R12, R02-R20, R10-R01]
 *   omega_user = -omega_internal
 */
static double extract_omega1_user_deg(Matrix3x3 R)
{
    ...
    double omega_int0 = factor * (R.m[2][1] - R.m[1][2]);
    return -omega_int0 * 180.0 / M_PI;  /* omega_user = -omega_internal */
}
...
int main(int argc, char *argv[])
{
    if (argc < 4) {
        fprintf(stderr,
            "使い方: %s <init_omega1_deg> <収束CSV> <サマリCSV>\n"
    ...
    /* 初期回転行列: omega_internal = (-init_w1, -30°, 0°) */
    double omega_init[3] = {
        -init_deg    * M_PI / 180.0,
        -TRUE_W2_DEG * M_PI / 180.0,
        -TRUE_W3_DEG * M_PI / 180.0
    ...
    double omega1_est = extract_omega1_user_deg(R);
    ...
    /* サマリCSV: init_deg,iter,final_E,frob_err,omega1_est_deg,R00..R22 */
    fprintf(fp_sum, "%.1f,%d,%.10f,%.8f,%.6f",
            init_deg, iter, E, frob_err, omega1_est);
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++)
