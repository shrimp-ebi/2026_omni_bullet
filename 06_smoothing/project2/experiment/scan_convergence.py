...
真値: ω₁=0, ω₂=-30°, ω₃=0
使用バイナリ: ./build/lm_convergence_cases_base2
...
SCAN_MIN       = 10.0             # スキャン下限
SCAN_MAX       = 20.0             # スキャン上限
STEP           = 0.1              # スキャン刻み幅 [度]
MAX_WORKERS    = 16                # 並列実行数
FROB_THRESHOLD = 0.005            # Frobenius誤差の収束判定しきい値（真値収束: Frob≈0）
TIMEOUT_SEC    = 240              # 1回の実行タイムアウト [秒]

BINARY     = "./build/lm_convergence_cases"
RESULT_CSV = "results/scan_C_omega1_true15_ref30.csv"
RESULT_PNG = "results/scan_C_omega1_true15_ref30.png"


# ── バイナリのビルド確認 ────────────────────────────────────────────────────
def build_binary():
    print("lm_convergence_cases_base3 をビルド中 (make lm_convergence_cases_base3)...")
    ret = subprocess.run(["make", "lm_convergence_cases_base3"],
...
def run_lm_single(init_deg):
    """
    lm_convergence_cases_base2 を init_deg で実行し結果を返す。
