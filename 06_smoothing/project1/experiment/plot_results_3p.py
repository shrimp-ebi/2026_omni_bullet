...
# -*- coding: utf-8 -*-
"""
3パラメータ版 omega2方向スキャン結果の可視化スクリプト

plot_results_2.py（Y軸版）と全く同じ形式でグラフを出力する。

- 期待角度は --expected-angle で指定可能
- 未指定時は results/expected_angle_3p.txt または 5.0° を使用
"""
...
def resolve_expected_angle(cli_value):
    ...
    metadata_path = 'results/expected_angle_3p.txt'
...
def main():
    parser = argparse.ArgumentParser(description='3パラメータ版omega2スキャン結果の可視化')
    parser.add_argument('--expected-angle', type=float, default=None,
                        help='期待される回転角度（度）。未指定時は results/expected_angle_3p.txt または 5.0 を使用')
    args = parser.parse_args()

    expected_angle = resolve_expected_angle(args.expected_angle)

    print('\n' + '=' * 60)
    print('3パラメータ版 omega2スキャン - 結果可視化')
    print('=' * 60)
    print(f'期待角度: {expected_angle:.2f}°\n')

    obj_csv   = 'results/objective_function_3p.csv'
    deriv_csv = 'results/derivatives_3p.csv'

    output_dir = 'results/graphs_3p'
    ...
    print('データを読み込み中...')
    for path in [obj_csv, deriv_csv]:
        if not os.path.exists(path):
            print(f'エラー: {path} が見つかりません')
            print('先に ./build/scan_omega2 を実行してください')
            return
