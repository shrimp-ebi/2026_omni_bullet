...
std::string resolve_data_root() {
    ...
    for (const auto& candidate : candidates) {
        const fs::path abs = fs::absolute(candidate);
        if (fs::exists(abs / "猫")) {
            return abs.string();
        }
    }

    return fs::absolute(cwd / ".." / "cmake-build-debug").string();
}

std::string make_path(const std::string& relative_path) {
    return (fs::path(resolve_data_root()) / relative_path).string();
}

void expand_mp4(const std::string& filename){
    // mp4形式の全天球動画読み込み
    cv::VideoCapture cap(make_path(filename + ".mp4"));
    if(!cap.isOpened()){
    ...
    while(true){
        ...
        cv::imwrite(make_path(filename + "/" + std::to_string(count) + ".png"), frame);
        count++;
    }
}
...
void reverse(const std::string& filename)
{
    ...
    while(true){
        // 入力画像を読み込む
        cv::Mat img_in = cv::imread(make_path(filename + "/" + std::to_string(count) + ".png"), cv::IMREAD_COLOR);
        if(count == 300){
            return;
        }
        ...
        cv::imwrite(make_path(filename + std::to_string(count) + ".png"), reversed);
        count+=30;
    }
}
...
void reverse2(const std::string& filename)
{
    ...
    while(true){
        // 入力画像を読み込む
        cv::Mat img_in = cv::imread(make_path(filename + "/0.png"), cv::IMREAD_COLOR);
        if(count == 10){
            return;
        }
...
/*       z_R.at<double>(0, 0) = cos(theta);
       ...
       z_R.at<double>(2, 2) = cos(theta);
        ...
        cv::imwrite(make_path(filename + "/roll/" + std::to_string(count) + ".png"), reversed);
...
void draw_gaze_line(){
    int count = 0;
    while(true){
        cv::Mat img_in_color = cv::imread(make_path("猫/test/" + std::to_string(count) + ".png"), cv::IMREAD_COLOR);
...
//        int cols = 6080;
        ...
        cv::imwrite(make_path("猫/test/line_" + std::to_string(count) + ".png"), img_in_color);
        count += 1;
    }
}
...
//    return 0;
    ...
    const std::string data_root = resolve_data_root();
    std::string homename = data_root + "/";
    ...
    while(count<=1) {
        ...
        // 入力画像を読み込む
        cv::Mat img_in = cv::imread(homename + filename + "/center/" + std::to_string(count) + ".png", cv::IMREAD_GRAYSCALE);
        cv::Mat img_in_color = cv::imread(homename + filename + "/center/" + std::to_string(count) + ".png", cv::IMREAD_COLOR);
