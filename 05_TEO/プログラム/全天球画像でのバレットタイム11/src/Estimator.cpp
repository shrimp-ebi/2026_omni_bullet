...
//                std::cout << uv_out[1] << "," << uv_out[0] << " : " << uv_in[1] << "," << uv_in[0] << std::endl;
    ...
    {
        cv::Mat R;
        for(double angle=-5; angle<=10; angle+=0.1){
        // 初期値設定
...
//        std::cout << R << std::endl;
          ...
          // 回転行列を生成
          R = cv::Mat::eye(3, 3, CV_64F); // 単位行列で初期化

          R.at<double>(0, 0) = cos(radians);
          R.at<double>(0, 1) = sin(radians);
          R.at<double>(1, 0) = -sin(radians);
          R.at<double>(1, 1) = cos(radians);
...
//        cv::imwrite("/home/h233304/全天球画像でのバレットタイム11/cmake-build-debug/猫/diff_image_x.png", diff_image_x);
//        cv::imwrite("/home/h233304/全天球画像でのバレットタイム11/cmake-build-debug/猫/diff_image_y.png", diff_image_y);
//        cv::imwrite("/home/h233304/全天球画像でのバレットタイム11/cmake-build-debug/猫/diff_image_z.png", diff_image_z);  */
...
//                 std::cout << "xy_out: " << xy_out[0] << "," << xy_out[1] << std::endl;
                    ...
                    cv::Vec3d X = Sb.ptr<cv::Vec3d>(xy_in[1])[xy_in[0]];
                    double differential_psi_x = -X[0]*std::sin(radians)+X[1]*std::cos(radians);
                    double differential_psi_y = -X[0]*std::cos(radians)-X[1]*std::sin(radians);
                    double differential_psi_z = 0;
...
//                    std::cout << "sx,sy,sz = " << double(sx) << ", " << double(sy) << ", " << double(sz) << std::endl;
                    ...
                    double nabla = sy * differential_psi_x + sz * differential_psi_y + sx * differential_psi_z;
...
/*                    grad[0] += error * nabla_e1;
            ...
            appendToCSV("/home/h233304/全天球画像でのバレットタイム11/cmake-build-debug/猫/data17.csv", angle, sum_of_error);
            appendToCSV("/home/h233304/全天球画像でのバレットタイム11/cmake-build-debug/猫/data18.csv", angle, grad);
...
//                appendToCSV("/home/h233304/全天球画像でのバレットタイム11/cmake-build-debug/猫/data2.csv", sum_of_error_new, cv::norm(delta_w));
                return R;
            }
...
/*        cv::imwrite("/home/h233304/全天球画像でのバレットタイム11/cmake-build-debug/猫/blurred.png", blurred);
	cv::imwrite("/home/h233304/全天球画像でのバレットタイム11/cmake-build-debug/猫/first_diff_image_x.png", first_diff_image_x);        
        cv::imwrite("/home/h233304/全天球画像でのバレットタイム11/cmake-build-debug/猫/first_diff_image_y.png", first_diff_image_y);*/
