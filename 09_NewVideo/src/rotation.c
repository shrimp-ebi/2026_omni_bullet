...
/* 注視点Gから回転行列を計算（printf なし版）
 *
 * 動画フレームごとに呼ばれるため、出力なしの専用関数を用意。
 * 計算ロジックは compute_rotation_matrix と同一。
 */
Matrix3x3 compute_rotation_matrix_quiet(Vector3D G) {
    Matrix3x3 R;
    Vector3D ez = compute_ez(G);
    Vector3D ex = compute_ex(ez);
    Vector3D ey = compute_ey(ez, ex);
...
/* 参照画像上での ∂S/∂θ, ∂S/∂φ を計算（差分カーネル、sigma/radius 可変版）
 *
 * deriv 方式で --sigma 引数を受け付けるために追加。
 * image_derivative_theta_phi と同じ計算で sigma と radius だけ外部から指定する。
 */
void image_derivative_theta_phi_sigma(Image *img, double u, double v,
    ...
    double *weight = (double *)malloc((radius + 1) * sizeof(double));
    double wsum = 0.0;
