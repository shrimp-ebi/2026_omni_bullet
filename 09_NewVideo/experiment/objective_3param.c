...
/* compute_gradient_3param と同じロジック。
 * image_derivative_theta_phi の代わりに image_derivative_theta_phi_sigma を呼ぶ。
 */
    ...
    double sigma, int radius)
{
    ...
    for (int v = v_min; v <= v_max; v++) {
        for (int u = u_min; u <= u_max; u++) {
            ...
            double dS_dtheta, dS_dphi;
            image_derivative_theta_phi_sigma(ref, u_ref, v_ref,
                                             &dS_dtheta, &dS_dphi,
                                             sigma, radius);

            double theta_p, phi_p;
            world_to_angle(Xp, &theta_p, &phi_p);
            double dth_dX, dth_dY, dth_dZ, dph_dX, dph_dY, dph_dZ;
            angle_jacobian_xyz(theta_p, phi_p,
                               &dth_dX, &dth_dY, &dth_dZ,
                               &dph_dX, &dph_dY, &dph_dZ);
