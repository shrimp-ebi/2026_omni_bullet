...
//                std::cout << "diff : " << abs(diff) << std::endl;
            ...
            double dw = cv::norm(delta_w);
            std::cout << "  iter " << std::setw(3) << count
                      << "  E=" << std::fixed    << std::setprecision(1) << sum_of_error_new
                      << "  Δ=" << std::scientific << std::setprecision(3) << dw << "\n";

            if(dw < 1.0e-5){
                std::cout << "  → 収束 (" << count << "回,  Δ=" << dw << ")\n";
                return R;
            }
